...
if ( isset($_POST['Submit']) ) // Soit c'est la 1ère sauvegarde, soit c'est au moins une sauvegarde supplémentaire
{
  ...
  $url = htmlspecialchars($_POST['URL']) ;
 	$objR->SetRSS($url,$_SESSION['current_user']->id ) ;
 	$res = $objR->Save() ;

	if ( $res == 1)
		echo "<script>alert('Flux RSS enregistré');</script>";

	if ( $res == -1)
		echo "<script>alert('Problème de sauvegarde');</script>";

  if ( $res == -2)
		echo "<script>alert('Flux RSS déjà existant');</script>";

  if ( $res == -3)
		echo "<script>alert('Flux RSS vide');</script>";
...
$fichier = $baseREP."/templates/RSS_create.html" ;
