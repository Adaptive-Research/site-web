...
$sql = "select a.Secteur, count(*) as nombre from Entreprises e left join Activity a on  e.idSecteur = a.id  where e.iscurrent = 1 and TypeClient = 'B2C' group by a.Secteur " ;
...
foreach ($stack as &$Secteur)
{
    ...
    if (!empty($Secteur))
      $sql = "select e.* from Entreprises e, Activity a where e.iscurrent = 1 and e.idSecteur = a.id  and a.Secteur like '%".$Secteur."%' and TypeClient = 'B2C' order by e.Entreprise asc" ;
    else
      $sql = "select * from Entreprises where iscurrent = 1  and (idSecteur is NULL or idSecteur = 0) and TypeClient = 'B2C' order by Entreprise asc " ;
