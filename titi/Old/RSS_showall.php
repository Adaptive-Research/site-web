...
#$contenu = $contenu.$servername." ".$username." ".$password." ".$dbname ;
...
$contenu = $contenu."<thead style=\"background-color:#AAAAAA\" > <tr>  <th style=\"width:5%\">Numéro</th> <th style=\"width:4%\">id</th> <th style=\"width:5%\">Effacer</th> <th>URL du Flux RSS</th> <th style=\"width:10%\">Dernière Récup.</th> <th>Titre du Flux RSS</th> </thead>\n";
...
$sql = "select * from RSS where iscurrent = 1 and TypeURL = 'RSS' order by id desc " ;
  ...
  while($row = $result->fetch_assoc())
  {
    ...
    if(isset($_SESSION['current_user']))
    {
      ...
      if ($objAC->IsVisibleBy('./RSS_delete.php',$_SESSION['current_user']->group_name))
        $contenu = $contenu."<td><a href=\"./delete-RSS/".$row['id']."/\"> <img src=\"./templates/img/b_drop.png\" title=\"Supprimer\" alt=\"Supprimer\"></a>";
      else
      	 $contenu = $contenu."<td>" ;
