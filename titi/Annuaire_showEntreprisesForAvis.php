...
$fichier = $baseREP."/templates/SelectAnnuaire1.html" ;
...
$sql = "select distinct temp.Titre, temp.id, temp.Telephone, temp.URL, temp.Ligne, temp.VilleMinu, temp.TypeEntreprise from ( " ;
$sql = $sql." SELECT  pe.* FROM Profession_Entreprises pe inner join Profession_Entreprises_ResultsRuns perr " ;
$sql = $sql." on perr.idEntreprise = pe.id  " ;
$sql = $sql." where pe.VilleMinu is not null and (pe.URL is not null and pe.URL <> '') " ;
if ($idVille != "")
  $sql = $sql." and pe.VilleMinu = \"".$idVille."\" " ;
$sql = $sql." and perr.NombreAvis = 0 " ;
$sql = $sql." order by Titre,perr.id ) as temp " ;
$sql = $sql." order by temp.Titre " ;
...
$contenu = $contenu."<thead style=\"background-color:#AAAAAA\" >  <th>Numéro</th>  <th style=\"width:20%\">Titre</th> <th style=\"width:10%\">Type</th> <th> Ville</th>  <th>Téléphone</th>  <th>Ligne</th>  </thead>\n";
